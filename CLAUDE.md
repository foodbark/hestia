...
- **Power settings that must hold:** wake timers enabled (AC+DC), hybrid sleep enabled (AC+DC), the S4 doze timeout (`9d7815a6-...`) set to `0`, and `CONSOLELOCK` (`SUB_NONE`) set to `0` so no password is required on wake — Hestia is a kiosk and must show the calendar with zero interaction. `powercfg /restoredefaultschemes` resets all of these — `hestia-reset.ps1` re-applies them explicitly after any restore.
